...
### Run the simplest version using Showcase to see as variables are changed
...
ui <- fluidPage(
  ...
  ### Input for site location
    ...
    ### Input for units
 	selectizeInput(
        'flow_units', 'Flow Units', choices = c(`Mean m3/s` = 'm3s', `Mean ft3/s` = 'cfs', `Total acre-ft` = 'ac-ft'),multiple = FALSE),  
         

   ### Input for subset
 	selectizeInput(
        'time_subset', 'Date Subset', choices = c(`Full` = '0', `January` = '1', `February` = '2', `March` = '3', `April` = '4', `May` = '5', `June` = '6', `July` = '7', `August` = '8', `September` = '9', `October` = '10', `November` = '11', `December` = '12' ),multiple = FALSE),  
...
paleo_ts <- reactive({
	if (list_id()=="None"){
		...
		subset_input <- as.numeric(input$time_subset)
		
		### Cut to months
		if (subset_input > 0) {
			paleo_ts_temp <- subset(paleo_ts_temp, Month==subset_input)
			start_year <- as.numeric(paleo_ts_temp$Year[[1]])
			### Remove Month and Year columns
			paleo_ts_temp <- paleo_ts_temp[ ,!(colnames(paleo_ts_temp) %in% c("Month", "Year"))]
			### Return time series
			ts(paleo_ts_temp, start=c(start_year), frequency=1)
		} else {
			start_month <- as.numeric(paleo_ts_temp$Month[[1]])
			start_year <- as.numeric(paleo_ts_temp$Year[[1]])
			### Remove Month and Year columns
			paleo_ts_temp <- paleo_ts_temp[ ,!(colnames(paleo_ts_temp) %in% c("Month", "Year"))]
			### Return time series
			ts(paleo_ts_temp, start=c(start_year, start_month), frequency=12)		
		}
	}
